    ...
    while true; do
        ...
        if [ -s "$STATE/cur" ] && \
           ...
           ! cmp -s "$STATE/cur" "$STATE/recv" 2>/dev/null; then
            ...
            ssh -o ConnectTimeout=2 -o StrictHostKeyChecking=accept-new "$VM" 'WAYLAND_DISPLAY=wayland-1 wl-copy' < "$STATE/cur" 2>/dev/null || true
        fi
        sleep 0.3
    done
) &
...
(
    while true; do
        ssh -o ConnectTimeout=2 -o StrictHostKeyChecking=accept-new "$VM" 'WAYLAND_DISPLAY=wayland-1 wl-paste 2>/dev/null' > "$STATE/remote" 2>/dev/null
        if [ -s "$STATE/remote" ] && \
